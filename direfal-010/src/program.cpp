...
void RefProgram::regModule(RefModuleBase *module){ // регистраци€ модул€ в программе (перед загрузкой)
...
RefChain*  RefProgram::executeExpression (RefChain *chain, Session *sess){ // вычисл€ет цепочку
	if (chain->isEmpty()) {
		return chain; // new RefChain();
	}
	//TODO: сохранить состо€ние ** стека аргументов дл€ условий
		...
		if (databr){
			if (ref_dynamic_cast<RefStructBrackets>(databr)){
				*result += new RefStructBrackets( executeExpression(databr->chain, sess) );
				...
				#endif
				//RefExecBrackets
				RefChain *arg = executeExpression(databr->chain, sess); // вычисл€ем внутренние exec-скобки
	...
	//TODO: откатить все аргументы дл€ условий до состо€ни€ **
return result;
};
...
RefData* RefProgram::createSymbolByCode(unistring code, unistring value){
	...
	while (modit != end){
		//TODO: поиск согласно пор€дку подключени€ модулей
		dllmod = dynamic_cast<RefDllModule*>(&(*modit->second));
		if (dllmod && (result=dllmod->constructSymbol(code, value))){
			return result;
		} // на данный момент в Ќ≈ dll-модул€х не может определ€тьс€ генератор символов по коду
		++modit;
	}
...
// создает переменную с именем по коду. »щет в подключенных модул€х (среди встроенных переменных и пользовательских шаблонов)
RefData* RefProgram::createVariableByTypename(unistring code, unistring value){
	...
	while (modit != end){
		//TODO: сделать поиск согласно пор€дку подключени€ модулей
		mod = &(*modit->second);
