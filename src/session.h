...
struct SessionStatePoint {
	...
	size_t count_repeats_idxs;	// не должен изменитьс€ после матчинга
...
class Session {
	...
	PooledTuple2 <RefData**, RefData**>  current_view_borders; // крайние view-элементы активного аргумента [дл€ скобок]
public:
...
public:
	...
	//todo: об€зательно переделать определение границ не через 0!!! иначе не по€нтно, кака€ из границ достигнута
	...
	//inline void setNewView(); - должно передаватьс€ в аргументе matching'а
	//inline void setNewTempl(RefChain* ch);

	void gc_prepare(RefData *save_point=0); // подготовка к сборке мусора
	inline void gc_exclude(RefData *data);         // исключение точки из удалени€
	void gc_exclude(RefChain *chain);              // исключение цепочки из удалени€
