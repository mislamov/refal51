		...
		if (databr){
			if (ref_dynamic_cast<RefStructBrackets>(databr)){
				*result += new RefStructBrackets( sess, executeExpression(databr->chain, sess) );
                ...
                if (! fresult->isEmpty()){
                    size_t idx = result->leng;  // сохраняем индекс элемента, который будет перед результатом
                                                // функции в поле зрения. Индекс вместо ссылки, так как realloc
					if (!idx) {
error:		--idx; // если цепочка была пуста, то в качестве ссылки запоминаем -1 (перед началом)
                    ...
                    *result += fresult;
                    ...
                    if (++iter < iend){ // если еще есть необработанный хвост
                        *result += new RefChain(sess, chain, iter, iend);  // сохраняем хвост
                    }
