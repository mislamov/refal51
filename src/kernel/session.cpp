...
TResult doGOandINIT(RefData *&activeTemplate, Session *s, SessionOfMaching *subsess) {
	...
	TResult result = activeTemplate->init(activeTemplate, s, r); /// ШАГ ВПЕРЕД
...
TResult doBACKandBACK(RefData *&activeTemplate, Session *s, SessionOfMaching *subsess) {
	...
	LOGSTEP("BACK");
	s->message4nextpred = mERROR;
	TResult result = activeTemplate->back(activeTemplate, s, l, r); /// ШАГ НАЗАД
...
/*
TResult doGOandINIT_old(RefData *&activeTemplate, Session *s, SessionOfMaching *subsess) {
RefData* &savedL = subsess->matching_savedL ;
RefData* &l =   subsess->matching_l ;
RefData* &r =   subsess->matching_r ;
...
TResult doBACKandBACK_old(RefData *&activeTemplate, Session *s, SessionOfMaching *subsess) {
RefData* &l =   subsess->matching_l ;
RefData* &r =   subsess->matching_r ;
RefData* &savedL =   subsess->matching_savedL ;
...
bool matchingBySession(Session *s, RefChain *tmplateChain, bool isdemaching) {
	...
	s->message4nextpred = mERROR;
	if (result_sost==GO) {
		doGOandINIT(activeTemplate, s, subsess);
	} else if (result_sost==BACK) {
		doBACKandBACK(activeTemplate, s, subsess);
	}
